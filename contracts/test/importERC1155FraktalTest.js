...
describe("FraktalNFT-ERC1155 handlers", function () {
  ...
  describe("Market Deployment", function () {
      ...
      it('Should mint fraktions to the minter', async function (){
        ...
        TokenImported = TokenLogicContract.attach(token1Address);
        if(logs) console.log(
          `Deployed a new ERC1155 FraktalNFT at: ${TokenImported.address}`,
        );
        let balances = await TokenImported.balanceOfBatch([alice.address,alice.address,market.address,market.address],[0,1,0,1]);
      ...
      it('should allow defraktionalize', async function () {
        ...
        await TokenImported.connect(alice).setApprovalForAll(market.address, true);
        if(logs) console.log('Alice defraktionalize');
        await market.connect(alice).defraktionalize(0);
        let balances = await TokenImported.balanceOfBatch([alice.address,alice.address,market.address],[0,1,0]);
      ...
      it('Should allow to lock ERC1155 tokens to the FraktalMarket.', async function () {
        if(logs) console.log('Alice imports its ERC1155');
        let importTx = await market.connect(alice).importERC1155(TokenImported.address, 0);
        let alice1Balance = await TokenImported.balanceOf(alice.address, 0);
        expect(alice1Balance).to.equal(ethers.BigNumber.from('0'));
        ...
        let collateralAddress = await market.getERC1155Collateral(Token1.address);
        expect(collateralAddress).to.equal(TokenImported.address);
      });
    it('should allow defraktionalize', async function () {
      if(logs) console.log('Alice approves the market');
      await Token1.connect(alice).setApprovalForAll(market.address, true);
      if(logs) console.log('Alice defraktionalize');
      await market.connect(alice).defraktionalize(1);
    ...
    it('Should not allow to claim the fraktal', async function () {
      ...
      await expect(
        market.connect(deedee).claimFraktal(1)
      ).to.be.revertedWith('not approval');
    ...
    it('Should allow to buy fraktions at new price', async function () {
      ...
      await market.connect(alice).buyFraktions(carol.address, 1, 100, {value: toPay(100, newPrice)});
    ...
    it('Should allow to claim the fraktal', async function () {
      ...
      await market.connect(deedee).claimFraktal(1);
      let balances = await Token1.balanceOfBatch([deedee.address, market.address],[0,0]);
    ...
    it('Should allow to whitdraw the locked nft', async function () {
      ...
      if(logs) console.log('Deedee whitdraws its ERC1155');
      await market.connect(deedee).claimERC1155(1);
      deedeeERC1155Balance = await TokenImported.balanceOf(deedee.address, 0);
      let deedeeBalance = await Token1.balanceOfBatch([deedee.address,deedee.address], [0,1]);
      expect(deedeeERC1155Balance).to.equal(ethers.BigNumber.from('1'));
      expect(deedeeBalance[1]).to.equal(ethers.BigNumber.from('0'));
      expect(deedeeBalance[0]).to.equal(ethers.BigNumber.from('0'));
      if(logs) console.log('Deedee has now the token ',TokenImported.address);
