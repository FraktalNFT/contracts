...
describe("Checks for ERC721", function () {
  ...
  it("Should able to receive NFTs and claim as fraktal owner (importERC721)", async function () {
    ...
    await erc721Fraktal.claimContainedERC1155(airdropERC1155.address,1,1)
    ...
    //approve factory for fraktalNFT
    await erc721Fraktal.setApprovalForAll(fraktalFactory.address,true)
    expect(await erc721Fraktal.isApprovedForAll(owner.address,fraktalFactory.address)).to.be.equal(true);
    await fraktalFactory.claimERC721(0);
    // console.log(await erc721.ownerOf(1));
    expect(await erc721.ownerOf(1)).to.be.equal(owner.address);
  });
  it("Should able to receive NFTs and claim as fraktal owner (importERC1155)", async function () {
    const [owner] = await ethers.getSigners();
    const FraktalNFT = await ethers.getContractFactory("FraktalNFT");
    const fraktalNFT = await FraktalNFT.deploy();
    await fraktalNFT.deployed();
    ...
    const SampleERC1155 = await ethers.getContractFactory("SampleERC1155");
    const erc1155 = await SampleERC1155.deploy();
    ...
    await erc1155.setApprovalForAll(fraktalFactory.address,true);
    
    const id = 1;
    const tx = await fraktalFactory.importERC1155(erc1155.address,id,8000);
    const eventArgs = ((await tx.wait()).events).find(e => e.event == "ERC1155Locked").args;

    const erc1155Fraktal = await FraktalNFT.attach(eventArgs.fraktal);

    //airdrop erc721
    // console.log("Owner of airdrop",await airdropERC721.ownerOf(1));
    await (airdropERC721['safeTransferFrom(address,address,uint256)'](owner.address,erc1155Fraktal.address,1))
    // console.log("After airdrop",await airdropERC721.ownerOf(1));

    //airdrop erc1155
    // console.log("FraktalNFT airdrop amount before airdrop:",(await airdropERC1155.balanceOf(erc721Fraktal.address,1)).toString());
    await airdropERC1155.safeTransferFrom(owner.address,erc1155Fraktal.address,1,1,[]);
    // console.log("FraktalNFT airdrop amount after airdrop:",(await airdropERC1155.balanceOf(erc721Fraktal.address,1)).toString());

    //claim airdropped tokens
    // console.log("Claim as airdrop Fraktal owner");
    await erc1155Fraktal.claimContainedERC721(airdropERC721.address,1)
    // console.log("Owner or ERC721 airdrop:",await airdropERC721.ownerOf(1));
    expect(await airdropERC721.ownerOf(1)).to.be.equal(owner.address);


    await erc1155Fraktal.claimContainedERC1155(airdropERC1155.address,1,1)
    // console.log("Owner of ERC1155 airdrop amount:",(await airdropERC1155.balanceOf(owner.address,1)).toString());
    expect(await airdropERC1155.balanceOf(owner.address,1)).to.be.equal(1);
    //approve factory for fraktalNFT
    await erc1155Fraktal.setApprovalForAll(fraktalFactory.address,true)
    expect(await erc1155Fraktal.isApprovedForAll(owner.address,fraktalFactory.address)).to.be.equal(true);
    await fraktalFactory.claimERC1155(0);
    // console.log(await erc721.ownerOf(1));
    expect(await erc1155.balanceOf(owner.address,1)).to.be.equal(1);
