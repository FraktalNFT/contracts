...
describe("FraktalNFT-ERC721 handlers", function () {
  ...
  describe("Market Deployment", function () {
      ...
      it('Should allow to lock ERC721 tokens to the FraktalFactory.', async function () {
        ...
        let importTx = await factory.connect(alice).importERC721(TokenERC721.address, 1, 6000);
        let tokenERC721owner = await TokenERC721.ownerOf(1);
        ...
        Token1 = FraktalImplementationContract.attach(importTokenAddress);
        if(logs) console.log(
        ...
        let collateralAddress = await factory.getERC721Collateral(Token1.address);
    ...
    //   await Token1.connect(bob).defraktionalize();
    //   let balances = await Token1.balanceOfBatch([bob.address,bob.address],[0,1]);
    //   expect(balances[0]).to.equal(ethers.BigNumber.from('1'));
    //   expect(balances[1]).to.equal(ethers.BigNumber.from('0'));
    // });
    ...
    it('should not allow to modify a listed item', async function(){
     await expect(
       market.connect(carol).listItem(
         Token1.address,//marketId
         newPrice, // total eth/amount
         100)
     ).to.be.revertedWith('unlist first');
    });
    ...
    it('Should sell if > majority', async function () {
      let majority = await Token1.majority();
      if(logs) console.log('Majority for token ',majority)

      if(logs) console.log('Alice approves the market to vote')
      await Token1.connect(alice).setApprovalForAll(market.address, true);
      if(logs) console.log('Alice votes on Deedee offer')
      await market.connect(alice).voteOffer(deedee.address, Token1.address);
      if(logs) console.log('Carol approves the market to vote')
      await Token1.connect(carol).setApprovalForAll(market.address, true);
      if(logs) console.log('Carol votes on Deedee offer')
      await market.connect(carol).voteOffer(deedee.address, Token1.address);

      let votesOnOffer = await market.getVotes(deedee.address, Token1.address);
      let balances = await Token1.balanceOfBatch([bob.address, alice.address, carol.address],[1,1,1]);
      let totalVotes = parseFloat(balances[0])+parseFloat(balances[1])+parseFloat(balances[2]);
      expect(votesOnOffer).to.equal(totalVotes);
      if(logs) console.log('Votes on offerer ',totalVotes)
    ...
    it('Should allow to whitdraw the locked nft', async function () {
      ...
      await Token1.connect(deedee).setApprovalForAll(factory.address, true);
      if(logs) console.log('Deedee whitdraws its ERC721');
      await factory.connect(deedee).claimERC721(0); // why tokenId!! its confusing! should be tokenAddress
      ...
      let itemAbandonedCollateral = await factory.getERC721Collateral(Token1.address);
